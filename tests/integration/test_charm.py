...
KNOWN_LIBRARY_INFO_OUTPUT.update(
    {
        Path('tests/library-austen-nested.zip'): KNOWN_LIBRARY_INFO_OUTPUT[
            Path('tests/library-austen-flat.zip')
        ]
    }
)
...
async def run_library_write_action(
    ops_test: OpsTest, behaviour: charm.LibraryWriteBehaviour | None = None
) -> None:
